        ...
        while (1) {
            ...
            while ((rv = recv(new_fd, buf, MAXDATASIZE-1, 0)) > 0) {
                ...
                if (strncmp(buf, "uTake ", 6) == 0) {
                  ...
                  sscanf(buf + 6, "%s", filename);
                  printf("Recieving file: %s\n", filename);

                  uint32_t net_size;
                  int bytes = recv(new_fd, &net_size, sizeof(net_size), 0);
                  if (bytes != sizeof(net_size)) {
                      fprintf(stderr, "Failed to receive file size.\n");
                      continue;
                  }
                  long file_size = ntohl(net_size);
                  ...
                  while (total_received < file_size) {
                      ssize_t bytes = recv(new_fd, buf, MAXDATASIZE, 0);
                      if (bytes <= 0) break;
                  ...
                  printf("Saved file to %s\n", fullpath);
                    ...
                    if (!fp) {
                        perror("fopen");
                        snprintf(buf, MAXDATASIZE, "ERROR: File not found\n");
                        send_all(new_fd, buf, strlen(buf));
                        continue;
                    }
                    ...
                    send_all(new_fd, &net_size, sizeof(net_size));

                    size_t bytes_read;
                    while ((bytes_read = fread(buf, 1, MAXDATASIZE, fp)) > 0) {
                        send_all(new_fd, buf, bytes_read);
                    }

                    fclose(fp);
                    printf("Sent file %s (%u bytes)\n", fullpath, file_size);
                } else {
                    for (int i = 0; buf[i]; i++) {
                        buf[i] = toupper(buf[i]);
                    }
